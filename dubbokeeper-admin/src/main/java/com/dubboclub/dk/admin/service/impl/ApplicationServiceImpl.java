...
public class ApplicationServiceImpl extends AbstractService implements ApplicationService {
	...
	public Map<String, Set<String>> listAllApplicationIps() {
		List<Provider> providers = this.providerService.listAllProvider();
		List<Consumer> consumers = this.consumerService.listAllConsumer();
		Map<String, Set<String>> result = new HashMap<String, Set<String>>();
		for (Provider provider : providers) {
			String key = provider.getApplication();
			if (StringUtils.isEmpty(key)) {
				key = provider.getAddress();
			}
			Set<String> ips = result.get(key);
			if (CollectionUtils.isEmpty(ips)) {
				ips = new HashSet();
				ips.add(provider.getAddress());
			} else {
				ips.add(provider.getAddress());
			}
			result.put(key, ips);
		...
		for (Consumer consumer : consumers) {
			String key = consumer.getApplication();
			if (StringUtils.isEmpty(key)) {
				key = consumer.getAddress();
			}
			Set<String> ips = result.get(key);
			if (CollectionUtils.isEmpty(ips)) {
				ips = new HashSet();
				ips.add(consumer.getAddress());
			} else {
				ips.add(consumer.getAddress());
			}
			result.put(key, ips);
